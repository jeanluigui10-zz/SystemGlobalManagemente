...
namespace System_Maintenance.Private.Report
{
    public partial class ReportCustomer : Page
    {
        ...
        private void CargaReporte()
        {
            ...
            List<Customer> list = CustomersBL.Instance.ListarCliente(ref objBase, fechaInicio, fechaFin);
        ...
        [WebMethod]
        public static object Cargar_Clientes(string fechaInicio, string fechaFin)
        {
            BaseEntity objBase = new BaseEntity();
            ...
            List<Customer> list = CustomersBL.Instance.ListarCliente(ref objBase, Inicio, Fin);
            if (objBase.Errors.Count == 0)
            {
                if (list != null)
                {
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            String fechaInicio = (String.IsNullOrEmpty(hfFechaInicio.Value)) ? "" : Convert.ToString(Convert.ToDateTime(hfFechaInicio.Value, CultureInfo.InvariantCulture).ToString("MM/dd/yyyy"));
            String fechaFin = (String.IsNullOrEmpty(hfFechaFin.Value)) ? "" : Convert.ToString(Convert.ToDateTime(hfFechaFin.Value, CultureInfo.InvariantCulture).ToString("MM/dd/yyyy"));
            List<CustomerExport> list = CustomersBL.Instance.ListarClienteExport(ref objBase, fechaInicio,fechaFin);
