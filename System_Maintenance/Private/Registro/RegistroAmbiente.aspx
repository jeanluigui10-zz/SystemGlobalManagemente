<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="RegistroAmbiente.aspx.cs" Inherits="System_Maintenance.Private.Registro.RegistroAmbiente" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <script type="text/javascript">
        ...
        function Fn_RecargarLista() {
            ...
            fn_callmethod("RegistroAmbiente.aspx/Recargar_Lista", "", success, error);
        }      

        function Fn_Registro()
        {
         
            if ($("input[id$=txtNombreAmbiente]").val().replace(/^\s+|\s+$/g, "").length == 0) {
                fn_message('i', 'Debe ingresar Nombre de ambiente.', 'message_row');
                return;
            }

             if ($("select[id$=ddlPiso] option:selected").val() == undefined || $("select[id$=ddlPiso] option:selected").val() == 0) {
                fn_message('i', 'Debe seleccionar Piso.', 'message_row');
                return;
            }
            ...
            obj = {
                Id_Piso :$("select[id$=ddlPiso] option:selected").val(),
                Nombre_Ambiente : $("input[id$=txtNombreAmbiente]").val().trim(),   
                Descripcion_Ambiente : $("input[id$=txtDescripcion]").val().trim(),  
                Estado : isChecked
            }
            var success = function (asw) {
                if (asw != null) {
            ...
            var error = function (xhr, ajaxOptions, thrownError) {
                fn_message('e', 'A ocurrido un error guardando el Ambiente');
            };

            var data = { obj: obj };

            fn_callmethod("RegistroAmbiente.aspx/Registro", JSON.stringify(data), success, error);
        }

        function Fn_Limpiar() {
            $("input[id$=txtNombreAmbiente]").val("");
    ...
    <div class="box box-warning">
        ...
        <div class="box-header with-border">
            <h3 <%--class="box-title"--%> style="text-align: center">Registro de Ambiente</h3>
        </div>
        <div class="box-body">
          <%--  <div class="form-group">
                <label>Nombre Ambiente</label>
                <asp:TextBox ID="txtNombreAmbiente" runat="server" type="text" CssClass="form-control"></asp:TextBox>
            </div>--%>
            <div class="form-group">
                <label>Nombre Ambiente</label>
                <asp:TextBox ID="txtNombreAmbiente" runat="server" type="text" CssClass="form-control"></asp:TextBox>
            ...
            <div class="form-group">
                <label>Piso</label>
                <asp:DropDownList ID="ddlPiso" runat="server" CssClass="form-control"></asp:DropDownList>
            </div>
