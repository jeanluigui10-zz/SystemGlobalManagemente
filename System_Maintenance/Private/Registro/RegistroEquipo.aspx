<%@ Page Title="" Language="C#" MasterPageFile="~/Home.Master" AutoEventWireup="true" CodeBehind="RegistroEquipo.aspx.cs" Inherits="System_Maintenance.Private.Registro.RegistroEquipo" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <script type="text/javascript">
        ...
        function Fn_RecargarLista() {
            ...
            fn_callmethod("RegistroEquipo.aspx/Recargar_Lista", "", success, error);
        }      

        function Fn_Registro()
        {
         
            if ($("input[id$=txtNombreEquipo]").val().replace(/^\s+|\s+$/g, "").length == 0) {
                fn_message('i', 'Debe ingresar Nombre de Equipo.', 'message_row');
                return;
            }

             if ($("select[id$=ddlCategoria] option:selected").val() == undefined || $("select[id$=ddlCategoria] option:selected").val() == 0) {
                fn_message('i', 'Debe seleccionar Categoria.', 'message_row');
                return;
            }
            ...
            obj = {
                Id_Categoria :$("select[id$=ddlCategoria] option:selected").val(),
                Nombre_Equipo : $("input[id$=txtNombreEquipo]").val().trim(),   
                Descripcion_Equipo : $("input[id$=txtDescripcion]").val().trim(),  
                Estado : isChecked
            }
            var success = function (asw) {
                if (asw != null) {
            ...
            fn_callmethod("RegistroEquipo.aspx/Registro", JSON.stringify(data), success, error);
        }

        function Fn_Limpiar() {
            $("input[id$=txtNombreEquipo]").val("");
    ...
    <div class="box box-warning">
        ...
        <div class="box-header with-border">
            <h3 <%--class="box-title"--%> style="text-align: center">Registro de Equipo</h3>
        </div>
        <div class="box-body">
          <%--  <div class="form-group">
                <label>Nombre Ambiente</label>
                <asp:TextBox ID="txtNombreEquipo" runat="server" type="text" CssClass="form-control"></asp:TextBox>
            </div>--%>
            <div class="form-group">
